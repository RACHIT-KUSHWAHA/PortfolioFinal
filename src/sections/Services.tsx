...
export function Services() {
  ...
  <Reveal className="mt-10 grid gap-4 sm:gap-6 md:grid-cols-2">
        {features.map((f) => (
          <div
            key={f.text}
            className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-5 text-left shadow-sm shadow-black/10"
            role="group"
            aria-label={f.text}
          >
            ...
            <div className="text-[15px] font-medium text-white/90">{f.text}</div>
          </div>
        ))}
  </Reveal>
    </section>
