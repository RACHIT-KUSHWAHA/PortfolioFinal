...
export const Impact: React.FC = () => {
  ...
  <Reveal className="mt-10 grid gap-6 sm:grid-cols-2 lg:mt-16 lg:grid-cols-4">
        {items.map((it) => (
          <div
            key={it.label}
            className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-lg shadow-black/10 backdrop-blur-sm"
            role="group"
            aria-label={it.label}
          >
            <div className="inline-flex items-center justify-center rounded-xl bg-emerald-500/10 p-2 text-emerald-400">
              {it.icon}
            </div>
            <div className="mt-4 text-3xl font-semibold text-white"><CountUp value={it.value} /></div>
            <div className="mt-1 text-sm text-zinc-400">{it.label}</div>
          </div>
        ))}
  </Reveal>
    </section>
