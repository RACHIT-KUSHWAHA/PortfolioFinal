...
export function Projects() {
  ...
  <Reveal className="grid gap-3 sm:gap-4 md:grid-cols-2 lg:grid-cols-3">
          {list.map((p, idx) => {
                ...
                <Reveal delay={idx * 0.03}>
                  <TiltCard>
                    <div className="overflow-hidden rounded-lg border border-white/5">
                      <img src={p.image} alt={p.title} className="aspect-video w-full object-cover transition-transform duration-300 will-change-transform hover:scale-[1.03]" loading="lazy" decoding="async" />
                    </div>
                  </TiltCard>
                </Reveal>
              </Wrapper>
